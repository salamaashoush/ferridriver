...
impl BrowserContext {
    ...
    pub async fn new_page(&self) -> Result<Page> {
        let page = Box::pin(self.inner.new_page()).await.map_err(napi::Error::from_reason)?;
        Ok(Page::wrap(page))
    }
    ...
    pub async fn add_cookies(&self, cookies: Vec<CookieData>) -> Result<()> {
        let native: Vec<ferridriver::backend::CookieData> = cookies.iter()
            .map(ferridriver::backend::CookieData::from).collect();
    ...
    pub async fn delete_cookie(&self, name: String, domain: Option<String>) -> Result<()> {
        ...
        ctx.delete_cookie(&name, domain.as_deref()).await.map_err(napi::Error::from_reason)
    }
    ...
    pub async fn grant_permissions(&self, permissions: Vec<String>, origin: Option<String>) -> Result<()> {
        self.inner.grant_permissions(&permissions, origin.as_deref())
    ...
    pub async fn set_geolocation(&self, latitude: f64, longitude: f64, accuracy: Option<f64>) -> Result<()> {
        self.inner.set_geolocation(latitude, longitude, accuracy.unwrap_or(1.0))
            .await.map_err(napi::Error::from_reason)
    }
