...
impl McpClient {
    fn new(backend: &str) -> Self {
        ...
        let mut child = Command::new(&binary)
            .arg("mcp").arg("--backend").arg(backend)
            .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::null())
            .spawn()
            .unwrap_or_else(|e| panic!("Failed to start: {binary}: {e}"));
        let stdout = child.stdout.take().unwrap();
        let stdin = child.stdin.take().unwrap();
        let mut c = McpClient { child, reader: BufReader::new(stdout), stdin };
        c.initialize();
    ...
    fn send_request(&mut self, method: &str, params: Value) -> Value {
        ...
        loop {
            ...
            if resp.get("id").and_then(|v| v.as_u64()) == Some(id) { return resp; }
        }
    }
...
fn test_evaluate_dom(c: &mut McpClient) {
    c.nav("<h1>Test</h1>");
    let t = c.tool_text("evaluate", json!({"expression": "document.querySelector('h1').textContent"}));
...
fn test_evaluate_large_payload(c: &mut McpClient) {
    c.nav("<body></body>");
    let t = c.tool_text("evaluate", json!({"expression": "JSON.stringify(Array(1000).fill('x'))"}));
...
fn test_screenshot_full_page(c: &mut McpClient) {
    ...
    assert!(b64.len() > 1000, "full page PNG should be substantial: {} bytes", b64.len());
}
...
fn test_click_selector(c: &mut McpClient) {
    c.nav("<h1 id='h'>Before</h1><button id='btn' onclick=\"document.getElementById('h').textContent='After'\">Go</button>");
    c.call_tool("click", json!({"selector": "#btn"}));
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('h').textContent"}));
...
fn test_click_at(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('d').textContent"}));
...
fn test_press_key(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('t').value.length"}));
    let len: i64 = t.trim().parse().unwrap_or(0);
    assert!(len > 0, "press Enter in textarea should insert newline, value length: {len}");
}
...
fn test_reload(c: &mut McpClient) {
    ...
    c.call_tool("evaluate", json!({"expression": "document.body.textContent = 'modified'"}));
    ...
    assert!(after.contains("original"), "reload should restore original content: {after}");
}
...
fn test_console_messages(c: &mut McpClient) {
    ...
    assert!(t.starts_with('[') || t.contains("hello123") || t.contains("log"),
...
fn test_network_requests(c: &mut McpClient) {
    ...
    assert!(t.contains("example.com") || t.contains("GET") || t.starts_with('['),
...
fn test_drag(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('d').dataset.down"}));
...
fn test_trace(c: &mut McpClient) {
    ...
    c.call_tool("evaluate", json!({"expression": "for(let i=0;i<1000;i++) Math.sqrt(i)"}));
    let t = c.tool_text("trace_stop", json!({}));
    assert!(t.contains("Metrics") || t.contains("Trace stopped"), "trace should return metrics: {t}");
}
...
fn test_fill_form(c: &mut McpClient) {
    ...
    c.call_tool("fill_form", json!({"fields":[
...
fn test_get_dropdown_options(c: &mut McpClient) {
    ...
    assert!(t.contains("Red") && t.contains("Green") && t.contains("Blue"), "should list options: {t}");
}
...
fn test_fill_dispatches_events(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('r').textContent"}));
...
fn test_click_offscreen_element(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('b').textContent"}));
    assert!(t.contains("clicked"), "click should scroll and click offscreen element: {t}");
}
...
fn test_selector_chain(c: &mut McpClient) {
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.querySelector('.a button').textContent"}));
...
fn test_selector_placeholder(c: &mut McpClient) {
    ...
    c.call_tool("fill", json!({"selector": "placeholder=Enter your name", "value": "Alice"}));
...
fn test_file_upload(c: &mut McpClient) {
    ...
    let r = c.call_tool("upload_file", json!({
        "selector": "#f",
    ...
    // Verify file count
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('f').files.length"}));
    assert!(t.contains("1"), "file count should be 1: {t}");

    // Verify file name
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('f').files[0].name"}));
    assert!(t.contains("ferridriver_test_upload.txt"), "file name should match: {t}");

    // Verify file size (17 bytes = "test file content")
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('f').files[0].size"}));
    ...
    let t = c.tool_text("evaluate", json!({"expression": "document.getElementById('r').textContent"}));
    assert!(t.contains("name:ferridriver_test_upload.txt"), "change event should fire with correct name: {t}");
    assert!(t.contains("content:test file content"), "FileReader should read correct content: {t}");
    ...
    macro_rules! run {
        ($name:ident) => {{
            ...
            match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| $name(&mut c))) {
                Ok(()) => { passed += 1; }
                Err(_) => { failed += 1; failures.push(name.to_string()); eprintln!("  FAIL {name}"); }
            }
        }};
    }
