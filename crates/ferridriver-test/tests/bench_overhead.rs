...
async fn measure_operation_costs() {
  ...
  println!("  Context+Page create:   {:>6.1}ms (avg of {n})", ctx_total.as_millis() as f64 / n as f64);
  ...
  println!("  Navigate (data URL):   {:>6.1}ms (avg of {n})", nav_total.as_millis() as f64 / n as f64);
  ...
  println!("  Read title:            {:>6.1}ms (avg of {n})", title_total.as_millis() as f64 / n as f64);

  // 5. Locator click (10x average)
  page.goto(&"data:text/html,<button id='b' onclick=\"\">Go</button>".to_string(), None).await.unwrap();
  ...
  println!("  Locator click:         {:>6.1}ms (avg of {n})", click_total.as_millis() as f64 / n as f64);
  ...
  println!("  Text content:          {:>6.1}ms (avg of {n})", text_total.as_millis() as f64 / n as f64);
