...
async fn deep_profile() {
  ...
  println!("      Single browser launch:   {:>7.1}ms", t.elapsed().as_secs_f64() * 1000.0);
  ...
  let b2 = b2.unwrap(); let b3 = b3.unwrap(); let b4 = b4.unwrap();
  println!("      3 browsers parallel:     {:>7.1}ms (amortized: {:.1}ms each)",
    t.elapsed().as_secs_f64() * 1000.0,
    t.elapsed().as_secs_f64() * 1000.0 / 3.0);
  b2.close().await.ok(); b3.close().await.ok(); b4.close().await.ok();
  println!();
  ...
  println!("      ctx+page combined:       {:>7.2}ms avg, p50={:.2}ms, p95={:.2}ms", total_avg, total_p50, total_p95);
  println!();
  ...
  println!("      data URL goto:           {:>7.2}ms avg, p50={:.2}ms, p95={:.2}ms", nav_avg, nav_p50, nav_p95);
  ...
  for _ in 0..iters {
    let t = Instant::now(); page.title().await.unwrap(); title_times.push(t.elapsed());
    let t = Instant::now(); page.locator("#b").click().await.unwrap(); click_times.push(t.elapsed());
    let t = Instant::now(); page.locator("#b").text_content().await.unwrap(); text_times.push(t.elapsed());
    let t = Instant::now(); page.evaluate("1+1").await.unwrap(); eval_times.push(t.elapsed());
  }
  ...
  println!("      context.close():         {:>7.2}ms avg, p50={:.2}ms, p95={:.2}ms",
    avg_ms(&close_times), percentile(&close_times, 50), percentile(&close_times, 95));
  println!();
  ...
  for i in 0..iters {
    ...
    let ctx = b1.new_context();
    let page = ctx.new_page().await.unwrap();
    let url = data_url(&format!("<button id='b' onclick=\"this.textContent='done'\">Click {i}</button>"));
    page.goto(&url, None).await.unwrap();
  ...
  println!("      Full cycle:              {:>7.2}ms avg, p50={:.2}ms, p95={:.2}ms", cycle_avg, cycle_p50, cycle_p95);
  println!();
  ...
  println!("  [7] Dispatch channel overhead ({iters}k iterations)");
  {
    ...
    let n = iters * 1000;
    let t = Instant::now();
    for i in 0..n as u32 { let _ = tx.send(i).await; }
    for _ in 0..n { let _ = rx.recv().await; }
  ...
  let plan = TestPlan {
    suites: vec![TestSuite {
      name: "noop".into(), file: "bench".into(),
      tests: vec![noop_test],
      hooks: Hooks::default(), annotations: Vec::new(),
      mode: SuiteMode::Parallel,
    }],
    total_tests: 1, shard: None,
  };
  ...
  println!("  ────────────────────────────────");
  println!("      context + page creation: {:>6.2}ms  (irreducible CDP cost)", total_avg);
  println!("      navigation (data URL):   {:>6.2}ms", nav_avg);
  println!("      click + text_content:    {:>6.2}ms", avg_ms(&click_times) + avg_ms(&text_times));
  println!("      context close:           {:>6.2}ms", avg_ms(&close_times));
  println!("      ─────────────────────────────────");
  println!("      Total per-test (serial): {:>6.2}ms", cycle_avg);
  println!("      Theoretical 50 tests / 4 workers: {:.0}ms", cycle_avg * 50.0 / 4.0);
  println!();
