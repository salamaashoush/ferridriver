...
const FILES: &[(&str, &str)] = &[
  (
    "login.js",
    "globalThis.exports = { name: 'acme.login', description: 'login', \
  ...
  (
    ...
    "const V = 'yes';\nglobalThis.exports = { tools: [\
       { name: 'acme.noop', description: 'noop', exposeAsTool: true, async handler() { return null; } },\
       { name: 'acme.setFeatureFlip', description: 'ff', exposeAsTool: true, \
         async handler({ args }) { \
...
async fn plugin_path_bench() {
  ...
  let names: Vec<Vec<&str>> = vec![
    vec!["acme.login"],
    vec!["acme.noop", "acme.setFeatureFlip"],
