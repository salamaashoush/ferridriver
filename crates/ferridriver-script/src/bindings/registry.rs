...
mod tests {
  ...
  fn no_ceiling_keeps_declared_semantics() {
    ...
    let e = effective_net(v(&["api.acme.com"]), None).expect("declared list");
    assert_eq!(&*e, ["api.acme.com".to_string()].as_slice());
  }

  #[test]
  fn ceiling_flips_undeclared_tools_to_the_ceiling() {
    let e = effective_net(Vec::new(), Some(&v(&["*.acme.com"]))).expect("ceiling");
    assert_eq!(&*e, ["*.acme.com".to_string()].as_slice());
  }

  #[test]
  fn ceiling_intersects_declared_entries() {
    let e = effective_net(
      v(&["api.acme.com", "evil.example", "*.acme.com"]),
      Some(&v(&["*.acme.com"])),
    )
    .expect("intersection");
    assert_eq!(&*e, v(&["api.acme.com", "*.acme.com"]).as_slice());
  }
  ...
  fn empty_ceiling_denies_everything() {
    ...
    assert!(e.is_empty());
    let e = effective_net(v(&["api.acme.com"]), Some(&[])).expect("deny-all");
    assert!(e.is_empty());
  }

  #[test]
  fn subsumption_covers_exact_wildcard_and_apex() {
    let ceiling = v(&["*.acme.com", "localhost"]);
    assert!(net_entry_subsumed("api.acme.com", &ceiling));
    assert!(net_entry_subsumed("acme.com", &ceiling), "wildcard covers the apex");
    assert!(net_entry_subsumed("localhost", &ceiling));
    assert!(net_entry_subsumed("*.acme.com", &ceiling), "equal wildcard");
    assert!(net_entry_subsumed("*.api.acme.com", &ceiling), "narrower wildcard");
    ...
    assert!(
      !net_entry_subsumed("evilacme.com", &v(&["*.acme.com"])),
