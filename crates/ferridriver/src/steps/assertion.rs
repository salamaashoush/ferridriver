...
async fn inner_text(page: &crate::backend::AnyPage, sel: &str) -> Result<String, String> {
    let r = page
        .evaluate(&format!(
...
async fn js_bool(page: &crate::backend::AnyPage, expr: &str) -> Result<bool, String> {
...
step!(Visible {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        super::find(page, &sel).await.map_err(|_| format!("'{sel}' is not visible"))?;
        Ok(None)
    }
});
...
step!(NotVisible {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        if super::find(page, &sel).await.is_ok() {
