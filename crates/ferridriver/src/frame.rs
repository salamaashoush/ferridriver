...
impl Frame {
    ...
    pub fn get_by_role(&self, role: &str, opts: &RoleOptions) -> Locator {
        ...
        Locator { page: self.page.clone(), selector: sel, frame_id: Some(self.id.clone()) }
    }
    ...
    pub fn get_by_test_id(&self, test_id: &str) -> Locator {
        Locator { page: self.page.clone(), selector: format!("testid={test_id}"), frame_id: Some(self.id.clone()) }
    }
    ...
    pub async fn goto(&self, url: &str) -> Result<(), String> {
        if self.is_main_frame() {
            ...
            self.evaluate(&format!("window.location.href = '{}'", url.replace('\'', "\\'"))).await?;
            Ok(())
        }
    }
