...
impl std::fmt::Display for ClickGuardError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::IsSelect => write!(f, "Cannot click <select> directly. Use select_option or get_dropdown_options instead."),
            Self::IsFileInput => write!(f, "Cannot click file input directly. Use evaluate() to set files programmatically."),
        }
    }
}
...
pub async fn suggest_selectors(page: &AnyPage) -> Vec<String> {
    let json_str = rt_eval_str(page, "window.__fd.suggestSelectors()").await.unwrap_or_default();
                ...
                if is_empty().await {
                    return Err(
                        "Page loaded but DOM is empty. The page may need JS rendering, \
                         ...
                         Try wait_for with a selector, or try a different URL.".into()
                    );
                }
            }
        }
...
pub async fn search_page(page: &AnyPage, opts: &SearchOptions) -> Result<SearchResult, String> {
    ...
    Ok(SearchResult { matches, total, has_more })
}
...
pub async fn find_elements(page: &AnyPage, opts: &FindElementsOptions) -> Result<FindResult, String> {
    ...
    if selectors::is_rich_selector(&opts.selector) {
        ...
        let elements = matched.into_iter().take(opts.max_results).map(|m| {
            FoundElement {
                index: m.index,
                tag: m.tag,
    ...
    let js = format!(
        "window.__fd.findElementsCSS({selector}, {attributes}, {max_results}, {include_text})"
    );
    ...
    for el in &result.elements {
        ...
        if let Some(text) = &el.text {
            if !text.is_empty() {
                ...
                let display = if display.len() > 120 { format!("{}...", &display[..120]) } else { display };
...
pub async fn select_option(
    ...
    let result_json = page.evaluate("(function() { \
...
pub async fn wait_for_actionable(element: &AnyElement, page: &AnyPage) -> Result<(), String> {
    ...
    loop {
        ...
        let val = page.evaluate("(function() { \
