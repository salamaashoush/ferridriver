...
impl CdpRawBrowser {
    ...
    pub async fn launch(chromium_path: &str) -> Result<Self, String> {
        Box::pin(Self::launch_with_flags(chromium_path, &crate::state::chrome_flags(true, &[]))).await
    }
    ...
    pub async fn launch_with_flags(chromium_path: &str, flags: &[String]) -> Result<Self, String> {
        ...
        let (transport, child) =
            Box::pin(WsTransport::spawn(chromium_path, &user_data_dir, flags)).await?;
    ...
    pub async fn connect(ws_url: &str) -> Result<Self, String> {
        ...
        if let Some(targets) = result.get("targetInfos").and_then(|t| t.as_array()) {
            for target in targets {
                if target.get("type").and_then(|v| v.as_str()) == Some("page") {
                    let target_id = target.get("targetId").and_then(|v| v.as_str()).unwrap_or("").to_string();
                    let attach = transport
                        .send_command(None, "Target.attachToTarget", serde_json::json!({"targetId": target_id, "flatten": true}))
                        .await?;
                    let sid = attach.get("sessionId").and_then(|v| v.as_str()).map(std::string::ToString::to_string);
        ...
        if !found_page {
            let create_result = transport
                ...
                .await?;
            let target_id = create_result.get("targetId").and_then(|v| v.as_str()).unwrap_or("").to_string();
            let attach = transport
                .send_command(None, "Target.attachToTarget", serde_json::json!({"targetId": target_id, "flatten": true}))
                .await?;
            let sid = attach.get("sessionId").and_then(|v| v.as_str()).map(std::string::ToString::to_string);
...
impl CdpRawPage {
    ...
    async fn history_go(&self, delta: i32) -> Result<(), String> {
        ...
        let current = hist.get("currentIndex").and_then(serde_json::Value::as_i64).unwrap_or(0);
        ...
        let Some(target_idx) = usize::try_from(target).ok().filter(|&t| t < entries.len()) else { return Ok(()); };
        let entry_id = entries[target_idx].get("id").and_then(serde_json::Value::as_i64).unwrap_or(0);
            ...
            while let Ok(event) = rx.recv().await {
                ...
                if event.get("method").and_then(|m| m.as_str()) == Some("Runtime.consoleAPICalled") {
                    if let Some(params) = event.get("params") {
                        ...
                        let text = params.get("args").and_then(|a| a.as_array())
                            .map(|args| args.iter().filter_map(|a| a.get("value").map(|v| v.to_string().trim_matches('"').to_string())).collect::<Vec<_>>().join(" "))
                            .unwrap_or_default();
                    ...
                    "Network.requestWillBeSent" => {
                        if let Some(params) = event.get("params") {
                            ...
                            let net_req = NetRequest {
                                ...
                                status: None, mime_type: None, headers, post_data,
                            };
                    ...
                    "Network.responseReceived" => {
                        if let Some(params) = event.get("params") {
                            ...
                            emitter.emit(crate::events::PageEvent::Response(crate::events::NetResponse {
                                request_id: rid, url, status: status.unwrap_or(0), status_text,
                                mime_type: mime.unwrap_or_default(), headers: resp_headers,
                            }));
                        }
                    }
            ...
            while let Ok(event) = rx.recv().await {
                ...
                if event.get("method").and_then(|m| m.as_str()) == Some("Page.javascriptDialogOpening") {
                    if let Some(params) = event.get("params") {
                        ...
                        let _ = transport.send_command(session_id.as_deref(), "Page.handleJavaScriptDialog", cmd_params).await;
                        ...
                        dialog_log.write().await.push(crate::state::DialogEvent {
                            dialog_type: dialog_type.to_string(), message: message.clone(),
        ...
        tokio::spawn(async move {
            ...
            while let Ok(event) = rx.recv().await {
                ...
                match method {
                    ...
                    "Runtime.executionContextsCleared" => { frame_contexts.write().await.clear(); }
                    "Page.frameAttached" => {
                        if let Some(params) = event.get("params") {
    ...
    async fn ensure_binding_channel(&self) -> Result<(), String> {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(event) = rx.recv().await {
                ...
                if event.get("method").and_then(|m| m.as_str()) != Some("Runtime.bindingCalled") { continue; }
                if let Some(params) = event.get("params") {
                    if params.get("name").and_then(|v| v.as_str()) != Some("__fd_binding__") { continue; }
    ...
    async fn ensure_fetch_enabled(&self) -> Result<(), String> {
        ...
        tokio::spawn(async move {
            ...
            while let Ok(event) = rx.recv().await {
                ...
                if event.get("method").and_then(|m| m.as_str()) != Some("Fetch.requestPaused") { continue; }
                ...
                let headers: rustc_hash::FxHashMap<String, String> = req_obj
                    .and_then(|r| r.get("headers")).and_then(|h| h.as_object())
                    .map(|obj| obj.iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect())
                    .unwrap_or_default();

                let intercepted = crate::route::InterceptedRequest {
                    request_id: request_id.clone(), url: url.clone(), method, headers, post_data, resource_type,
                };

                let action = {
                    let routes_guard = routes.read().await;
                    routes_guard.iter().find(|r| r.pattern.is_match(&url)).map(|r| (r.handler)(&intercepted))
                };
                ...
                match action {
                    Some(crate::route::RouteAction::Fulfill(resp)) => {
                        ...
                        let mut hdrs: Vec<serde_json::Value> = resp.headers.iter()
                            .map(|(k, v)| serde_json::json!({"name": k, "value": v})).collect();
                        ...
                        let _ = t.send_command(sid.as_deref(), "Fetch.fulfillRequest", serde_json::json!({
                    ...
                    Some(crate::route::RouteAction::Continue(overrides)) => {
                        ...
                        if let Some(u) = &overrides.url { p["url"] = serde_json::Value::String(u.clone()); }
                        if let Some(m) = &overrides.method { p["method"] = serde_json::Value::String(m.clone()); }
                        if let Some(h) = &overrides.headers {
                            p["headers"] = serde_json::Value::Array(h.iter().map(|(k,v)| serde_json::json!({"name":k,"value":v})).collect());
                        }
                    ...
                    Some(crate::route::RouteAction::Abort(reason)) => {
                        let er = match reason.to_lowercase().as_str() {
                            "aborted" => "Aborted", "blockedbyclient" => "BlockedByClient",
                            "connectionfailed" => "ConnectionFailed", "connectionrefused" => "ConnectionRefused",
                            "internetdisconnected" => "InternetDisconnected", "timedout" => "TimedOut",
                            _ => "Failed",
                        };
                        let _ = t.send_command(sid.as_deref(), "Fetch.failRequest",
                            serde_json::json!({"requestId": request_id, "errorReason": er})).await;
                    }
                    None => {
                        let _ = t.send_command(sid.as_deref(), "Fetch.continueRequest",
    ...
    pub async fn route(&self, pattern: &str, handler: crate::route::RouteHandler) -> Result<(), String> {
        ...
        self.routes.write().await.push(crate::route::RegisteredRoute {
            pattern: regex, pattern_str: pattern.to_string(), handler,
        });
