...
impl CdpRawBrowser {
    ...
    pub async fn launch(chromium_path: &str) -> Result<Self, String> {
        let user_data_dir =
            std::env::temp_dir().join(format!("ferridriver-raw-{}", std::process::id()));
            ...
            for target in targets {
                if target.get("type").and_then(|v| v.as_str()) == Some("page") {
                    ...
                    let attach = transport
                        ...
                        .await?;
                    let sid = attach.get("sessionId").and_then(|v| v.as_str()).map(|s| s.to_string());
                    Self::enable_domains(&transport, sid.as_deref()).await?;
                    attached.insert(target_id, sid.clone());
