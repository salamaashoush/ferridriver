    ...
    pub async fn pages(&self) -> Result<Vec<AnyPage>, String> {
        ...
        match r {
            IpcResponse::ViewList(ids) => Ok(ids.into_iter().map(|id| {
                AnyPage::WebKit(WebKitPage { client: self.client.clone(), view_id: id, events: crate::events::EventEmitter::new(), routes: std::sync::Arc::new(std::sync::RwLock::new(Vec::new())), closed: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)) })
            }).collect()),
    ...
    pub async fn new_page(&self, url: &str) -> Result<AnyPage, String> {
        ...
        match r {
            IpcResponse::ViewCreated(id) => {
                let page = WebKitPage { client: self.client.clone(), view_id: id, events: crate::events::EventEmitter::new(), routes: std::sync::Arc::new(std::sync::RwLock::new(Vec::new())), closed: std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false)) };
...
impl WebKitPage {
    ...
    fn ok(r: IpcResponse) -> Result<(), String> {
        match r { IpcResponse::Ok | IpcResponse::Value(_) | IpcResponse::ViewCreated(_) | IpcResponse::ViewList(_) | IpcResponse::Binary(_) => Ok(()), IpcResponse::Error(e) => Err(e) }
    }
    ...
    pub fn pdf(&self, _landscape: bool, _print_background: bool) -> impl std::future::Future<Output = Result<Vec<u8>, String>> {
    ...
    pub async fn accessibility_tree_with_depth(&self, depth: i32) -> Result<Vec<AxNodeData>, String> {
        ...
        Ok(raw.iter().map(|n| AxNodeData {
            ...
            parent_id: n.get("parentId").and_then(|v| v.as_str()).map(std::string::ToString::to_string),
            backend_dom_node_id: None,
            ignored: n.get("ignored").and_then(serde_json::Value::as_bool).unwrap_or(false),
            role: n.get("role").and_then(|v| v.as_str()).map(std::string::ToString::to_string),
            name: n.get("name").and_then(|v| v.as_str()).filter(|s| !s.is_empty()).map(std::string::ToString::to_string),
            description: n.get("description").and_then(|v| v.as_str()).filter(|s| !s.is_empty()).map(std::string::ToString::to_string),
    ...
    pub fn grant_permissions(&self, _permissions: &[String], _origin: Option<&str>) -> impl std::future::Future<Output = Result<(), String>> {
        let result = if self.closed.load(std::sync::atomic::Ordering::Relaxed) {
            Err("Page is closed".into())
        } else {
            Ok(())
    ...
    pub async fn metrics(&self) -> Result<Vec<MetricData>, String> {
        ...
        let r = self.evaluate(js).await?;
        let s = r.and_then(|v| v.as_str().map(std::string::ToString::to_string)).unwrap_or("[]".into());
            ...
            loop {
                ...
                // Drain console events
                {
                    let msgs: Vec<(String, String)> = {
                        let Ok(mut log) = client.console_log.lock() else { continue };
                ...
                // Drain dialog events
                {
                    let evts: Vec<(String, String, String)> = {
                        let Ok(mut log) = client.dialog_log.lock() else { continue };
                        if log.is_empty() { Vec::new() } else { std::mem::take(&mut *log) }
                    };
                    if !evts.is_empty() {
                ...
                // Drain network events
                {
                    let evts: Vec<(String, String, String, String)> = {
                        let Ok(mut log) = client.network_log.lock() else { continue };
                        if log.is_empty() { Vec::new() } else { std::mem::take(&mut *log) }
                    };
                    if !evts.is_empty() {
                        ...
                        for (id, method, url, resource_type) in evts {
                            let req = NetRequest {
                                ...
                                resource_type: resource_type.clone(), status: None, mime_type: None,
    ...
    pub fn expose_function(&self, name: &str, _func: crate::events::ExposedFn) -> impl std::future::Future<Output = Result<(), String>> {
        let result = if self.closed.load(std::sync::atomic::Ordering::Relaxed) {
            Err("Page is closed".into())
        } else {
    ...
    pub fn remove_exposed_function(&self, name: &str) -> impl std::future::Future<Output = Result<(), String>> {
        let result = if self.closed.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            Err(format!("remove_exposed_function('{name}') not yet supported on WebKit backend"))
        };
        std::future::ready(result)
    }
    ...
    pub async fn route(&self, pattern: &str, handler: crate::route::RouteHandler) -> Result<(), String> {
        ...
        self.routes.write().map_err(|e| format!("routes write lock poisoned: {e}"))?.push(crate::route::RegisteredRoute {
            pattern: regex.clone(),
        ...
        {
            let mut rh = self.client.route_handler.lock().map_err(|e| format!("route_handler lock poisoned: {e}"))?;
            if rh.is_none() {
                let routes_ref = self.routes.clone();
                *rh = Some(std::sync::Arc::new(move |url: &str, method: &str, headers_json: &str, post_data: &str| {
                    let Ok(routes) = routes_ref.read() else { return r#"{"action":"continue"}"#.to_string() }; // read lock -- hot path
                    for route in routes.iter() {
                        if route.pattern.is_match(url) {
                            ...
                            let intercepted = crate::route::InterceptedRequest {
                                ...
                                headers,
                                post_data: if post_data.is_empty() { None } else { Some(post_data.to_string()) },
                            ...
                            return match action {
                                ...
                                crate::route::RouteAction::Continue(_) => {
        ...
        let js = format!("(function(){{window.__fd_routes=window.__fd_routes||[];window.__fd_routes.push(new RegExp('{regex_str}'))}})();");
        self.evaluate(&js).await?;
    ...
    pub async fn unroute(&self, pattern: &str) -> Result<(), String> {
        ...
        self.routes.write().map_err(|e| format!("routes write lock poisoned: {e}"))?.retain(|r| r.pattern_str != pattern);

        // Remove from JS-side pattern list
        if let Ok(regex) = crate::route::glob_to_regex(pattern) {
            let regex_str = regex.as_str().replace('\\', "\\\\").replace('\'', "\\'");
            let js = format!("(function(){{window.__fd_routes=(window.__fd_routes||[]).filter(function(r){{return r.source!=='{regex_str}'}})}})()");
            self.evaluate(&js).await?;
        }
        Ok(())
    }
