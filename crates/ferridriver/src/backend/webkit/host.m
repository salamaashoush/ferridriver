...
static void dispatch_frame(uint32_t req_id, uint8_t op,
    ...
    switch (op) {
        case OP_CREATE_VIEW: {
            ...
            // `Runtime.exceptionThrown` (CDP) / `log.entryAdded type=javascript`
            // (BiDi) — routes a synthesised `{name, message, stack}` through
            // the existing fdConsole IPC frame with a distinguishing
            // `level: 'pageerror'` marker. The Rust-side drainer routes
            // `'pageerror'` levels to `PageEvent::PageError(WebError)`
            // instead of `PageEvent::Console(ConsoleMessage)`.
            //
            // Stock `WKWebView` has no native delegate for uncaught JS
            // errors, so we rely on `window.addEventListener('error', …)` /
            // `'unhandledrejection'`. Payload shape is intentionally
            // `name: ': '-separated` combined into `text` so it round-trips
            // through the existing (level, text) IPC without protocol
            // changes — Section B gap carried from §2.12.
            NSString *errorJS = @"(function(){if(window.__fd_err)return;window.__fd_err=1;"
                "var h=webkit.messageHandlers.fdConsole;"
                ...
                "try{var reason=e.reason;"
                "var name='Error';var msg=String(reason);var stack='';"
