...
fn decode_shm_screenshot(payload: &[u8]) -> IpcResponse {
    ...
    let bytes = (|| -> Result<Vec<u8>, String> {
        ...
        unsafe {
            ...
            let map = libc::mmap(std::ptr::null_mut(), png_len, libc::PROT_READ,
...
impl IpcClient {
    ...
    pub async fn spawn() -> Result<(Self, std::process::Child), String> {
        ...
        let dialog_log: Arc<StdMutex<Vec<(String, String, String)>>> =
            Arc::new(StdMutex::new(Vec::new()));
        let network_log: Arc<StdMutex<Vec<NetworkEvent>>> =
            Arc::new(StdMutex::new(Vec::new()));
        let route_handler: Arc<StdMutex<Option<RouteCallback>>> =
        ...
        let client = Self {
            writer: StdMutex::new(write_sock), pending, next_id: AtomicU64::new(1),
            console_log, dialog_log, network_log, event_notify, route_handler,
        };
    ...
    fn spawn_host_process(
        ...
        let child = unsafe {
            ...
            cmd.spawn().map_err(|e| format!("spawn webkit host ({}): {e}", exe.display()))?
        };
        Ok(child)
    }
            ...
            loop {
                ...
                let response = match rep {
                    ...
                    8 => {
                        ...
                        if let Ok(mut log) = ctx.console_log.lock() { log.push((level, text)); }
                    ...
                    9 => {
                        ...
                        if let Ok(mut log) = ctx.dialog_log.lock() { log.push((dtype, message, action)); }
                        ctx.event_notify.notify_one();
                        continue;
                    }
                    10 => {
                        ...
                        if let Ok(mut log) = ctx.network_log.lock() { log.push((id, method, url, res_type)); }
                        ctx.event_notify.notify_one();
                        continue;
                    }
                    11 => {
    ...
    pub async fn send(&self, op: Op, payload: &[u8]) -> Result<IpcResponse, String> {
        ...
        self.pending.lock().map_err(|e| format!("pending lock poisoned: {e}"))?.insert(u64::from(rid), tx);
        {
        ...
        match tokio::time::timeout(std::time::Duration::from_secs(30), rx).await {
            ...
            Err(_) => {
                if let Ok(mut guard) = self.pending.lock() { guard.remove(&u64::from(rid)); }
                Err("timeout".into())
            }
        }
    }
