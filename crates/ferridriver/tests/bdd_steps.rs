...
async fn bdd_wait_steps() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<div id='container'></div><script>setTimeout(function(){document.getElementById('container').innerHTML='<span id=\"loaded\">Done</span>'},200)</script>",
  );
  ...
  // Test wait for text
  let url2 = data_url(
    "<div id='msg'></div><script>setTimeout(function(){document.getElementById('msg').textContent='Ready'},200)</script>",
  );
...
async fn bdd_wait_timeout_steps() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<div id='area'></div><script>setTimeout(function(){document.getElementById('area').innerHTML='<span id=\"delayed\">here</span>'},300);setTimeout(function(){document.getElementById('area').textContent='final text'},600)</script>",
  );
...
async fn bdd_assertion_visibility_text_value() {
  ...
  let page = browser.page().await.expect("page");
  let url =
    data_url("<div id='visible'>Hello</div><input id='inp' type='text' value='test123'><h1 id='heading'>Welcome</h1>");
...
async fn bdd_assertion_attributes_classes() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<a id='link' href='/about' class='nav active' data-id='42'>About</a><input id='req' type='text' required>",
  );
...
async fn bdd_assertion_state() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<button id='ok'>OK</button><button id='nope' disabled>No</button><input id='cb' type='checkbox' checked><input id='cb2' type='checkbox'>",
  );
...
async fn bdd_variable_steps() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<title>Var Test</title><h1 id='heading'>Hello World</h1><input id='inp' type='text' value='secret123'><a id='link' href='/dashboard'>Go</a>",
  );
...
async fn bdd_wrong_count_fails() {
  ...
  let result = scenario::run(page.inner(), &scenario(&url, "  Then there should be 5 'li'"), opts())
    .await
    .unwrap();
  assert_failed(&result);
...
async fn bdd_click_at_coordinates() {
  ...
  let page = browser.page().await.expect("page");
  let url = data_url(
    "<div id='target' onclick=\"this.textContent='hit'\" style='position:absolute;left:50px;top:50px;width:100px;height:100px;background:#ccc'>click me</div>",
  );
...
async fn bdd_evaluate_returns_result() {
  ...
  let result = scenario::run(page.inner(), &scenario(&url, "  When I evaluate '2 + 3'"), opts())
    .await
    .unwrap();
  assert_passed(&result);
...
async fn bdd_detailed_failure_messages() {
  ...
  let r = scenario::run(
    page.inner(),
    &scenario(&url, "  Then the title should be 'WrongTitle'"),
    opts(),
  )
  .await
  .unwrap();
  assert_failed(&r);
  assert!(
    r.steps[1].error.as_ref().unwrap().contains("ActualTitle"),
    "error should show actual title"
  );
  assert!(
    r.steps[1].error.as_ref().unwrap().contains("WrongTitle"),
    "error should show expected title"
  );

  // Test text mismatch error
  let r = scenario::run(
    page.inner(),
    &scenario(&url, "  Then '#box' should have text 'wrong text'"),
    opts(),
  )
  .await
  .unwrap();
  assert_failed(&r);
  assert!(
    r.steps[1].error.as_ref().unwrap().contains("actual text"),
    "error should show actual text"
  );

  // Test value mismatch error
  let r = scenario::run(
    page.inner(),
    &scenario(&url, "  Then '#inp' should have value 'wrong_val'"),
    opts(),
  )
  .await
  .unwrap();
  assert_failed(&r);
  assert!(
    r.steps[1].error.as_ref().unwrap().contains("actual_val"),
  ...
  let r = scenario::run(
    page.inner(),
    &scenario(&url, "  Then '#box' should not be visible"),
    opts(),
  )
  .await
  .unwrap();
  assert_failed(&r);
  assert!(
    r.steps[1].error.as_ref().unwrap().contains("visible"),
...
fn bdd_registry_self_test() {
  ...
  assert!(
    reference.contains("Navigation"),
    "reference should contain Navigation category"
  );
  assert!(
    reference.contains("Assertion"),
    "reference should contain Assertion category"
  );
