...
async fn page_api_tests() {
  let browser = Browser::launch(LaunchOptions {
    ...
    ..Default::default()
  }).await.expect("launch browser");
  let page = browser.page().await.expect("get page");

  // ── Navigation ──
  page.goto(&data_url("<title>Hello</title><body>World</body>"), None).await.unwrap();
  ...
  // ── Locator click ──
  page.goto(&data_url("<button id='b' onclick=\"this.textContent='clicked'\">Go</button>"), None).await.unwrap();
  ...
  // ── get_by_role ──
  page.goto(&data_url("<button>Save</button><button>Cancel</button>"), None).await.unwrap();
  let count = page.get_by_role("button", &RoleOptions::default()).count().await.unwrap();
  assert_eq!(count, 2, "get_by_role count");

  // ── get_by_text ──
  page.goto(&data_url("<p>Hello World</p><p>Goodbye</p>"), None).await.unwrap();
  let count = page.get_by_text("Hello", &TextOptions::default()).count().await.unwrap();
  assert_eq!(count, 1, "get_by_text count");

  // ── get_by_label ──
  page.goto(&data_url("<label for='e'>Email</label><input id='e' type='email'>"), None).await.unwrap();
  ...
  // ── get_by_test_id ──
  page.goto(&data_url("<div data-testid='card'>Content</div>"), None).await.unwrap();
  let t = page.get_by_test_id("card").text_content().await.unwrap();
  assert!(t.unwrap_or_default().contains("Content"), "get_by_test_id");

  // ── Locator chaining ──
  page.goto(&data_url("<div class='a'><span>Inside A</span></div><div class='b'><span>Inside B</span></div>"), None).await.unwrap();
  let t = page.locator("css=.a").locator("css=span").text_content().await.unwrap();
  assert!(t.unwrap_or_default().contains("Inside A"), "chain");

  // ── first / last / nth ──
  page.goto(&data_url("<ul><li>A</li><li>B</li><li>C</li></ul>"), None).await.unwrap();
  ...
  // ── Visibility ──
  page.goto(&data_url("<div id='v'>visible</div><div id='h' style='display:none'>hidden</div>"), None).await.unwrap();
  assert!(page.locator("#v").is_visible().await.unwrap(), "visible");
  assert!(page.locator("#h").is_hidden().await.unwrap(), "hidden");

  // ── Enabled / disabled ──
  page.goto(&data_url("<input id='e'><input id='d' disabled>"), None).await.unwrap();
  assert!(page.locator("#e").is_enabled().await.unwrap(), "enabled");
  assert!(page.locator("#d").is_disabled().await.unwrap(), "disabled");

  // ── Checked ──
  page.goto(&data_url("<input type='checkbox' id='c'>"), None).await.unwrap();
  ...
  // ── innerHTML / innerText ──
  page.goto(&data_url("<div id='d'><b>Bold</b> text</div>"), None).await.unwrap();
  ...
  // ── Content + markdown ──
  page.goto(&data_url("<h1>Title</h1><p>Body text</p>"), None).await.unwrap();
  ...
  // ── all_text_contents ──
  page.goto(&data_url("<ul><li>Alpha</li><li>Beta</li><li>Gamma</li></ul>"), None).await.unwrap();
  ...
  // ── wait_for_function ──
  page.goto(&data_url("<script>setTimeout(function(){window.ready=true},200)</script>"), None).await.unwrap();
  ...
  // ── select_option ──
  page.goto(&data_url("<select id='s'><option value='a'>Apple</option><option value='b'>Banana</option></select>"), None).await.unwrap();
  ...
  // ── filter ──
  page.goto(&data_url("<div><p>Keep</p></div><div><p>Remove</p></div>"), None).await.unwrap();
  ...
  page.goto(&data_url("<body><script>document.title=window.innerWidth+'x'+window.innerHeight</script></body>"), None).await.unwrap();
...
async fn snapshot_for_ai_tests() {
  ...
  // ── Basic snapshot ──
  page.goto(&data_url("<h1>Hello World</h1><button>Submit</button><a href='#'>Link</a>"), None).await.unwrap();
  ...
  assert!(result.full.contains("heading"), "should contain heading role: {}", result.full);
  ...
  assert!(result.incremental.is_none(), "no incremental on first call without track");
  assert!(!result.ref_map.is_empty(), "ref_map should have entries");

  // ── Page header includes URL and title ──
  page.goto(&data_url("<title>Test Title</title><body>Content</body>"), None).await.unwrap();
  let result = page.snapshot_for_ai(SnapshotOptions::default()).await.unwrap();
  assert!(result.full.contains("Title: Test Title"), "should contain page title: {}", result.full);
  assert!(result.full.contains("URL: data:"), "should contain URL");

  // ── Depth limiting ──
  page.goto(&data_url("<div><ul><li><a href='#'>Deep Link</a></li></ul></div>"), None).await.unwrap();
  let deep = page.snapshot_for_ai(SnapshotOptions { depth: None, ..Default::default() }).await.unwrap();
  let shallow = page.snapshot_for_ai(SnapshotOptions { depth: Some(2), ..Default::default() }).await.unwrap();
  // Deep should have more content than shallow
  assert!(deep.full.len() >= shallow.full.len(),
    "unlimited depth ({}) should be >= depth=2 ({})", deep.full.len(), shallow.full.len());

  // ── Incremental tracking: first call ──
  page.goto(&data_url("<h1>V1</h1><button>Click</button>"), None).await.unwrap();
  let r1 = page.snapshot_for_ai(SnapshotOptions {
    track: Some("t1".to_string()),
    ..Default::default()
  }).await.unwrap();
  assert!(r1.full.contains("V1"), "first call should have V1");
  assert!(r1.incremental.is_none(), "first call with track should have no incremental");

  // ── Incremental tracking: content changed ──
  page.goto(&data_url("<h1>V2</h1><button>Click</button>"), None).await.unwrap();
  let r2 = page.snapshot_for_ai(SnapshotOptions {
    track: Some("t1".to_string()),
    ..Default::default()
  }).await.unwrap();
  ...
  let r3 = page.snapshot_for_ai(SnapshotOptions {
    track: Some("t1".to_string()),
    ..Default::default()
  }).await.unwrap();
  ...
  page.goto(&data_url("<button id='b1'>Save</button><a href='#'>Help</a>"), None).await.unwrap();
  let result = page.snapshot_for_ai(SnapshotOptions::default()).await.unwrap();
...
async fn add_init_script_tests() {
  ...
  page.goto(&data_url("<script>document.title = window.__test_init || 'missing'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  assert_eq!(title, "injected", "init script should set window.__test_init before page script runs");
  ...
  page.goto(&data_url("<script>document.title = (window.__test_init || '') + ':' + (window.__test_init2 || '')</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  ...
  page.goto(&data_url("<script>document.title = (window.__test_init || 'gone') + ':' + (window.__test_init2 || '')</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
...
async fn dialog_handling_tests() {
  ...
  page.goto(&data_url("<script>alert('hello'); document.title = 'after_alert'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  assert_eq!(title, "after_alert", "alert should be auto-accepted");

  // Default behavior: auto-accept confirms
  page.goto(&data_url("<script>var r = confirm('sure?'); document.title = r ? 'yes' : 'no'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  assert_eq!(title, "yes", "confirm should be auto-accepted");

  // Default behavior: accept prompts with default value
  page.goto(&data_url("<script>var r = prompt('name?', 'default'); document.title = r || 'null'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  assert_eq!(title, "default", "prompt should be accepted with default value");

  // Custom handler: dismiss all
  page.set_dialog_handler(Arc::new(|_: &PendingDialog| DialogAction::Dismiss)).await;
  page.goto(&data_url("<script>var r = confirm('sure?'); document.title = r ? 'yes' : 'no'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
  ...
  page.goto(&data_url("<script>var r = prompt('name?'); document.title = r || 'null'</script>"), None).await.unwrap();
  let title = page.title().await.unwrap();
...
async fn add_script_style_tag_tests() {
  ...
  page.add_script_tag(None, Some("document.title = 'injected'"), None).await.unwrap();
  let title = page.title().await.unwrap();
  ...
  let color = page.evaluate_str("getComputedStyle(document.getElementById('box')).color").await.unwrap();
...
async fn expose_function_tests() {
  ...
  page.expose_function("double", Arc::new(|args| {
  ...
  page.goto(&data_url("<body></body>"), None).await.unwrap();
  let result = page.evaluate_str("(async () => { const r = await window.double(21); return String(r); })()").await.unwrap();
  assert_eq!(result, "42", "exposed function should return doubled value: {result}");

  // Expose a function that concatenates strings
  page.expose_function("greet", Arc::new(|args| {
    let name = args.first().and_then(|v| v.as_str()).unwrap_or("world");
    serde_json::json!(format!("Hello, {}!", name))
  })).await.unwrap();

  let result = page.evaluate_str("(async () => { return await window.greet('Rust'); })()").await.unwrap();
  assert_eq!(result, "Hello, Rust!", "greet function should work: {result}");

  // Exposed function persists across navigations
  page.goto(&data_url("<body></body>"), None).await.unwrap();
  let result = page.evaluate_str("(async () => { return String(await window.double(5)); })()").await.unwrap();
  assert_eq!(result, "10", "exposed function should persist across navigations: {result}");

  // Multiple arguments
  page.expose_function("add", Arc::new(|args| {
    let a = args.get(0).and_then(|v| v.as_f64()).unwrap_or(0.0);
    let b = args.get(1).and_then(|v| v.as_f64()).unwrap_or(0.0);
    serde_json::json!(a + b)
  })).await.unwrap();

  let result = page.evaluate_str("(async () => { return String(await window.add(3, 4)); })()").await.unwrap();
...
async fn wait_for_load_state_tests() {
  ...
  let state = page.evaluate_str("document.readyState").await.unwrap();
  assert!(state == "interactive" || state == "complete",
...
async fn locator_evaluate_tests() {
  ...
  let texts = page.locator("css=.item").evaluate_all("elements.map(function(e){return e.textContent})").await.unwrap();
  assert_eq!(texts, Some(serde_json::json!(["Alpha", "Beta", "Gamma"])));

  // evaluate with computed values
  let rect = page.locator("#title").evaluate("({w: el.offsetWidth, h: el.offsetHeight})").await.unwrap();
  assert!(rect.is_some());
  let r = rect.unwrap();
  assert!(r.get("w").and_then(|v| v.as_f64()).unwrap_or(0.0) > 0.0, "should have width");
...
async fn locator_set_checked_tap_select_text() {
  ...
  // set_checked
  page.goto(&data_url("<input id='cb' type='checkbox'><input id='inp' type='text' value='select me'>"), None).await.unwrap();
  assert!(!page.locator("#cb").is_checked().await.unwrap());
  page.locator("#cb").set_checked(true).await.unwrap();
  assert!(page.locator("#cb").is_checked().await.unwrap(), "should be checked after set_checked(true)");
  page.locator("#cb").set_checked(false).await.unwrap();
  assert!(!page.locator("#cb").is_checked().await.unwrap(), "should be unchecked after set_checked(false)");
  page.locator("#cb").set_checked(true).await.unwrap();
  page.locator("#cb").set_checked(true).await.unwrap(); // idempotent
  assert!(page.locator("#cb").is_checked().await.unwrap(), "double set_checked(true) should still be checked");
  ...
  assert_eq!(text.unwrap_or_default(), "tapped", "tap should fire touch/pointer events");
...
async fn storage_state_tests() {
  ...
  assert!(state.get("localStorage").is_some(), "state should have localStorage key");
...
async fn locator_or_and_tests() {
  ...
  page.goto(&data_url("<button id='a'>Alpha</button><span id='b'>Beta</span><div id='c'>Gamma</div>"), None).await.unwrap();
...
async fn network_interception_tests() {
  use ferridriver::route::{RouteAction, FulfillResponse, ContinueOverrides};
  use std::sync::Arc;
  ...
  page.route("**/mock-page", Arc::new(|_req| {
  ...
  assert!(body.contains("This page is mocked"), "body should contain mocked text: {body}");

  // 2. Fulfill with JSON -- mock an API on the now-mocked origin
  page.route("**/api/data", Arc::new(|_req| {
    RouteAction::Fulfill(FulfillResponse {
      status: 200,
  ...
  let result = page.evaluate_str(
    "(async () => { const r = await fetch('/api/data'); return await r.text(); })()"
  ).await.unwrap();
  assert!(result.contains("\"mocked\":true"), "API should return mocked JSON: {result}");

  // 3. Abort -- block specific requests
  page.route("**/blocked", Arc::new(|_req| {
...
async fn quick_wins_tests() {
  ...
  assert!(page.locator("#exists").is_attached().await.unwrap(), "existing element should be attached");
  assert!(!page.locator("#gone").is_attached().await.unwrap(), "missing element should not be attached");

  // ── goto with options ──
  page.goto(
    &data_url("<title>Opts</title><body>content</body>"),
    Some(GotoOptions { wait_until: Some("domcontentloaded".into()), timeout: Some(10000) }),
  ).await.unwrap();
  let title = page.title().await.unwrap();
  ...
  browser.close().await.unwrap();
  assert!(!browser.is_connected().await, "browser should be disconnected after close");
}
