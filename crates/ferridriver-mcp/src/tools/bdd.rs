...
impl McpServer {
  #[tool(
    name = "list_steps",
    description = "Show all step patterns supported by run_scenario. Call this before writing a scenario."
  )]
  async fn list_steps(&self) -> Result<CallToolResult, ErrorData> {
    Ok(CallToolResult::success(vec![Content::text(
      ferridriver::steps::StepRegistry::global().reference(),
    )]))
  }

  #[tool(
    name = "run_scenario",
    description = "Run a Gherkin scenario (Given/When/Then steps). All steps execute in one call. Call list_steps first to see available step patterns."
  )]
