...
impl McpServer {
    #[tool(name = "cookies", description = "Manage cookies. Actions: get (list all), set (create/update), delete (remove by name), clear (remove all).")]
    async fn cookies(&self, Parameters(p): Parameters<CookiesParams>) -> Result<CallToolResult, ErrorData> {
        ...
        match p.action.as_str() {
            "get" => {
                ...
                Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&list).unwrap_or_default())]))
            }
            "set" => {
                ...
                Ok(CallToolResult::success(vec![Content::text(format!("Cookie '{name}' set."))]))
            }
            "delete" => {
                let name = p.name.as_deref().ok_or_else(|| Self::err("'name' required for delete"))?;
                page.delete_cookie(name, p.domain.as_deref()).await.map_err(Self::err)?;
                Ok(CallToolResult::success(vec![Content::text(format!("Cookie '{name}' deleted."))]))
            }
            "clear" => {
                page.clear_cookies().await.map_err(Self::err)?;
                Ok(CallToolResult::success(vec![Content::text("Cookies cleared.")]))
            }
            other => Err(Self::err(format!("Unknown action '{other}'. Use: get, set, delete, clear."))),
        }
    }
}
