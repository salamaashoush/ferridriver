...
use crate::params::{SnapshotParams, ScreenshotParams_, EvaluateParams, WaitForParams, SearchPageParams, SessionOnlyParams};
...
impl McpServer {
    #[tool(name = "snapshot", description = "Take an accessibility snapshot of the page. Supports depth limiting and incremental tracking.")]
    async fn snapshot(&self, Parameters(p): Parameters<SnapshotParams>) -> Result<CallToolResult, ErrorData> {
        ...
        match page.snapshot_for_ai(opts).await {
            ...
            Err(e) => Ok(CallToolResult::success(vec![Content::text(format!("[snapshot error: {e}]"))])),
        }
    }
    ...
    async fn evaluate(&self, Parameters(p): Parameters<EvaluateParams>) -> Result<CallToolResult, ErrorData> {
        ...
        let val = result.map_or_else(|| "undefined".to_string(), |v| serde_json::to_string_pretty(&v).unwrap_or_else(|_| v.to_string()));
    ...
    async fn search_page(&self, Parameters(p): Parameters<SearchPageParams>) -> Result<CallToolResult, ErrorData> {
        ...
        let result = ferridriver::actions::search_page(page.inner(), &opts).await.map_err(Self::err)?;
