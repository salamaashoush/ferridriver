...
impl McpServer {
    #[tool(name = "storage", description = "Manage localStorage. Actions: get (read key), set (write key=value), list (all entries), clear (remove all).")]
    async fn storage(&self, Parameters(p): Parameters<StorageParams>) -> Result<CallToolResult, ErrorData> {
        ...
        match p.action.as_str() {
            "get" => {
                ...
                let r = page.evaluate(&format!("localStorage.getItem('{}')", key.replace('\'', "\\'")))
                    .await.map_err(Self::err)?;
                Ok(CallToolResult::success(vec![Content::text(r.map_or("null".into(), |v| v.to_string()))]))
            }
            "set" => {
                let key = p.key.as_deref().ok_or_else(|| Self::err("'key' required for set"))?;
                let value = p.value.as_deref().ok_or_else(|| Self::err("'value' required for set"))?;
                page.evaluate(&format!("localStorage.setItem('{}', '{}')", key.replace('\'', "\\'"), value.replace('\'', "\\'")))
                    .await.map_err(Self::err)?;
                Ok(CallToolResult::success(vec![Content::text(format!("Set '{key}'='{value}'."))]))
            }
            "list" => {
                ...
                Ok(CallToolResult::success(vec![Content::text(serde_json::to_string_pretty(&parsed).unwrap_or_default())]))
            }
            "clear" => {
                page.evaluate("localStorage.clear()").await.map_err(Self::err)?;
                Ok(CallToolResult::success(vec![Content::text("localStorage cleared.")]))
            }
            other => Err(Self::err(format!("Unknown action '{other}'. Use: get, set, list, clear."))),
        }
    }
}
