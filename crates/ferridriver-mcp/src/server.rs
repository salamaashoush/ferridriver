...
use rmcp::{
    ...
    model::{CallToolResult, Content, ServerInfo, ServerCapabilities, SetLevelRequestParams, PaginatedRequestParams, ListResourcesResult, Resource, Annotated, RawResource, ReadResourceRequestParams, ReadResourceResult, ResourceContents, ListPromptsResult, Prompt, PromptArgument, GetPromptRequestParams, GetPromptResult, PromptMessage, PromptMessageRole},
...
impl McpServer {
    ...
    pub async fn page(&self, context: &str) -> Result<Page, ErrorData> {
        ...
        Box::pin(state.ensure_instance(&key.instance)).await.map_err(Self::err)?;
        let any_page = state
            .active_page(context)
            .map_err(Self::err)?
            .clone();
    ...
    pub async fn resolve(
        ...
        actions::resolve_element(page.inner(), ref_map, r#ref.map(String::as_str), selector.map(String::as_str)).await
    }
...
impl ServerHandler for McpServer {
    ...
    async fn list_resources(
        ...
        let result = ListResourcesResult { resources, ..Default::default() };
        Ok(result)
    }
    ...
    async fn read_resource(
        ...
        match resource.as_str() {
            "page-info" => {
                ...
                let json = serde_json::to_string_pretty(&serde_json::json!({"url": url, "title": title, "session": context_name})).unwrap_or_default();
                Ok(ReadResourceResult::new(vec![ResourceContents::text(json, uri).with_mime_type("application/json")]))
            }
            "console" => {
                let state = self.state.lock().await;
                let msgs = state.console_messages(&context_name, None, 100).await.map_err(Self::err)?;
                drop(state);
                let text = serde_json::to_string_pretty(&msgs).unwrap_or_default();
                Ok(ReadResourceResult::new(vec![ResourceContents::text(text, uri).with_mime_type("application/json")]))
            }
            "network" => {
                let state = self.state.lock().await;
            ...
            "screenshot" => {
                let bytes = page.screenshot(ferridriver::options::ScreenshotOptions::default()).await.map_err(Self::err)?;
    ...
    async fn list_prompts(&self, _request: Option<PaginatedRequestParams>, _context: RequestContext<RoleServer>) -> Result<ListPromptsResult, ErrorData> {
        ...
        let result = ListPromptsResult { prompts, ..Default::default() };
        Ok(result)
    }

    async fn get_prompt(&self, request: GetPromptRequestParams, _context: RequestContext<RoleServer>) -> Result<GetPromptResult, ErrorData> {
