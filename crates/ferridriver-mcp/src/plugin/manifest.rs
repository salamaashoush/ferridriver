...
pub struct PluginManifest {
  ...
  /// `tools` namespace (`tools.acme.login`) and as the MCP tool name when
...
pub struct PluginAllow {
  ...
  /// is an exact host (`api.acme.com`) or a leading-wildcard suffix
  /// (`*.acme.com`, which also matches the bare apex `acme.com`). Empty
