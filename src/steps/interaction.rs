...
step!(Click {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        let el = page.find_element(&sel).await.map_err(|e| format!("'{sel}': {e}"))?;
...
step!(ClickAt {
    ...
    execute(page, caps, _table, _vars) {
        ...
        page.click(chromiumoxide::layout::Point { x, y }).await.map_err(|e| format!("{e}"))?;
        Ok(None)
    }
});
...
step!(Fill {
    ...
    execute(page, caps, _table, _vars) {
        ...
        el.type_str(&val).await.map_err(|e| format!("{e}"))?;
        Ok(None)
    }
});
