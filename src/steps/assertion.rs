...
async fn inner_text(page: &chromiumoxide::Page, sel: &str) -> Result<String, String> {
    let r = page
        .evaluate(format!(
    ...
    Ok(r.value().and_then(|v| v.as_str()).unwrap_or("").to_string())
}

async fn js_bool(page: &chromiumoxide::Page, expr: &str) -> Result<bool, String> {
    let r = page.evaluate(expr).await.map_err(|e| format!("{e}"))?;
    Ok(r.value() == Some(&serde_json::Value::Bool(true)))
}
...
step!(PageHasText {
    ...
    execute(page, caps, _table, _vars) {
        let text = q(&caps[1]);
        if !js_bool(page, &format!("document.body?.innerText?.includes('{}') || false", js_escape(&text))).await? {
...
step!(Visible {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        page.find_element(&sel).await.map_err(|_| format!("'{sel}' is not visible"))?;
        Ok(None)
    }
});
...
step!(ContainsText {
    ...
    execute(page, caps, _table, _vars) {
        ...
        let text = inner_text(page, &sel).await?;
...
step!(ValueExact {
    ...
    execute(page, caps, _table, _vars) {
        ...
        let val = r.value().and_then(|v| v.as_str()).unwrap_or("").to_string();
        if val != expected {
...
step!(HasAttrValue {
    ...
    execute(page, caps, _table, _vars) {
        ...
        let r = page.evaluate(format!("document.querySelector('{}')?.getAttribute('{}') || ''", js_escape(&sel), js_escape(&attr)))
            .await.map_err(|e| format!("{e}"))?;
...
step!(HasAttr {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        let attr = q(&caps[2]);
        if !js_bool(page, &format!("document.querySelector('{}')?.hasAttribute('{}')", js_escape(&sel), js_escape(&attr))).await? {
...
step!(HasClass {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        let cls = q(&caps[2]);
        if !js_bool(page, &format!("document.querySelector('{}')?.classList.contains('{}')", js_escape(&sel), js_escape(&cls))).await? {
...
step!(Enabled {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        if !js_bool(page, &format!("!document.querySelector('{}')?.disabled", js_escape(&sel))).await? {
...
step!(Disabled {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        if !js_bool(page, &format!("document.querySelector('{}')?.disabled === true", js_escape(&sel))).await? {
...
step!(Checked {
    ...
    execute(page, caps, _table, _vars) {
        let sel = q(&caps[1]);
        if !js_bool(page, &format!("document.querySelector('{}')?.checked === true", js_escape(&sel))).await? {
...
step!(ElementCount {
    ...
    execute(page, caps, _table, _vars) {
        ...
        let r = page.evaluate(format!("document.querySelectorAll('{}').length", js_escape(&sel)))
            .await.map_err(|e| format!("{e}"))?;
        let actual = r.value().and_then(|v| v.as_u64()).unwrap_or(0) as usize;
        if actual != expected {
