...
# Full CI check
ready: fmt lint check-config-types test
  @echo "Ready to commit"

# Regenerate the TypeScript config-type declarations from the Rust schema (ts-rs).
config-types:
  #!/usr/bin/env bash
  set -euo pipefail
  cargo test -p ferridriver-config --lib
  out="packages/ferridriver-test/src/config-types/index.ts"
  {
    echo "// Auto-generated barrel; regenerated by 'just config-types'. Do not edit."
    echo ""
    find packages/ferridriver-test/src/config-types -name "*.ts" \
      -not -name "index.ts" -printf "%P\n" | sort | while read -r rel; do
      echo "export type * from './${rel%.ts}';"
    done
  } > "$out"
  echo "Config types written to packages/ferridriver-test/src/config-types/"

# Fail if the checked-in config types drifted from the current Rust schema.
check-config-types: config-types
  #!/usr/bin/env bash
  set -euo pipefail
  diff=$(git status --porcelain packages/ferridriver-test/src/config-types/)
  if [ -n "$diff" ]; then
    echo "Config type drift detected. Run 'just config-types' and commit:" >&2
    echo "$diff" >&2
    exit 1
  fi
