...
- Non-empty: the tool's `request` binding and `fetch` both flip to
  **default-deny**. Each entry is an exact host (`api.acme.com`) or a
  leading-wildcard suffix (`*.acme.com`, which also matches the bare apex
  `acme.com`). Any other host throws before the request is made. The
