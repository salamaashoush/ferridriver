...
| a boxed `test.step` is re-attributed, never hidden | `{ box: true }` rewrites the error's `stack` to the frames outside the step and reports the step at its caller's line, exactly as `worker/testInfo.ts:298-304` does. Playwright's reporter `TestStep` has no `box` field and no reporter drops a boxed step from the tree, so neither does ferridriver's. |
| the JSON report's steps carry no `location` | Playwright's `JSONReportTestStep` is `{title, duration, error, steps}` (`testReporter.d.ts:352-357`, `reporters/json.ts::_serializeTestStep`) — no location, even though the reporter API's `TestStep` has one. ferridriver matches the JSON shape rather than extending it; the location reaches consumers through the live event stream (`onStepBegin`), the blob report, the merged HTML report and the trace. |
| `stepInfo.titlePath` continues `testInfo.titlePath`, which a JS spec spells out per `describe` | Core builds a step's title path from the base its host reports. A `ferridriver test` spec passes the path it computed at discovery (one entry per `describe`); a Rust `#[ferritest]` test has only its `TestId`, whose suite is a single joined string, so its path is `[file, suite, test]`. Playwright always has the JS shape. |
