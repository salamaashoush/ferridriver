...
| gap | tests | fix |
|---|---|---|
| `browserType.launch({ proxy })` was accepted and never applied | — | The option reached `LaunchPlan` and stopped there: nothing in the engine read it, and both JS layers hardcoded `proxy: None` while their typings advertised it. It is now lowered per engine at launch — Chromium `--proxy-server`, WebKit `--proxy` (with `--ignore-host` per host on Linux, as Playwright spells it for the same GTK/WPE build), Firefox the WebDriver `proxy` capability in `session.new`, since it has no proxy switch at all. `ProxyConfig::launch_flags` is the single lowering; the config crate's instance `proxy` delegates to it, so a config-declared proxy and a launched one can never disagree. Credentials are never lowered to a flag. |
| a config-declared instance `proxy` was ignored on the WebKit backend | — | `ferridriver-config` lowered every instance proxy to Chromium's `--proxy-server`, which WebKit accepts and ignores — the proxy silently did nothing. Lowering is now backend-aware, and an instance's own `backend` overrides the section's when choosing the spelling. |
| WebKit lacked the Safari surface Playwright injects | — | The bundled build has no `window.safari`, no `window.GestureEvent` and no `PublicKeyCredential`, so Safari feature detection took the non-Safari branch. The document-start bootstrap now carries the same fragments `wkPage.ts::_calculateBootstrapScript` injects, including the desktop-only `delete window.orientation` / `ondevicemotion` / `ondeviceorientation`. |
| WebKit mobile emulation laid out at 980px and never enabled touch | — | `Emulation.setDeviceMetricsOverride` hardcoded `fixedLayout: false`, and `Page.setTouchEmulationEnabled`, `Page.overridePlatform` and the `isMobile` `Page.overrideSetting` family were never sent. All are wired to `ViewportConfig`, and `navigator.platform` now follows the emulated user agent the way `calculateUserAgentEmulation` derives it. |
...
| `fs.readFileSync` missing | 1 | The old bespoke `fs` was async-only. **Decision: `fs` is Node's** — the vendored `node:fs`, sync entry points and `fs.promises` alike, so a spec that reads a fixture or a download synchronously is ordinary. |
| reading `download.path()` was refused | 1 | A download lands in a backend-owned temp dir, which the old path jail treated as an escape. There is no jail: an absolute path is read as written, as in Node. |
