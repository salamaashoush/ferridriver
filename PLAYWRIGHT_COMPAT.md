...
- [x] **Full ElementHandle surface shipped on core + NAPI + QuickJS, with Rule-9 tests on all four backends.**
...
- **Known follow-ups** (do not block `[x]`; captured as separate work):
  - Full screenshot option bag at the backend path. The handle-level `screenshot_with_opts` accepts the full bag but today only honours `format` — completing `omitBackground`, `animations`, `mask`, `style`, `clip` requires threading `ScreenshotOpts` through `AnyPage::screenshot_element` on all four backends. Also gates Locator's screenshot surface.
  - `owner_frame` multi-frame attribution via per-backend primitives (CDP `DOM.describeNode`, BiDi context-id mapping). Current impl returns the main frame for any connected element — correct for single-frame pages.
  - `content_frame` full frame-tree attribution for inline iframes without name/id — today's impl matches by name/id/src attributes, which covers the common named-iframe case.
- **Playwright ref**: `packages/playwright-core/src/client/elementHandle.ts`.
- **Files**: `crates/ferridriver/src/element_handle.rs`; NAPI `crates/ferridriver-node/src/element_handle.rs`; QuickJS `crates/ferridriver-script/src/bindings/element_handle.rs`. Rule-9 tests in `crates/ferridriver-cli/tests/backends_support/handle_surface.rs` and `crates/ferridriver-node/test/handles.test.ts`.

### 1.3 JSHandle

- [x] **Full JSHandle surface shipped on core + NAPI + QuickJS, with Rule-9 tests on all four backends.**
- **Shipped surface**: `dispose`, `isDisposed`, `asElement` (probes `h instanceof Node` and re-wraps via `backend::element_from_remote`), `jsonValue` / `jsonValueWire`, `getProperty(name)`, `getProperties()`, `evaluate_with_arg` / `evaluate_with_arg_wire` / `evaluate_handle_with_arg`. Multi-arg `handle.evaluate(fn, userArg)` packs `[handle, userArg]` with proper `{h:i}` index shifting so the user function receives both as two positional parameters (matching Playwright's `javascript.ts:161-163`).
- **Dual backing (remote + value)**: `JSHandleBacking::Remote` carries a page-side reference (`Runtime.RemoteObjectId` / BiDi sharedRef or handle / WebKit `window.__wr` index); `JSHandleBacking::Value` carries an inline primitive so primitives returned from `evaluateHandle` round-trip via `JSHandle._value`-equivalent behaviour. Mirrors Playwright's dual shape at `server/javascript.ts:130-139`; all three backends emit the right shape (CDP reads `objectId` vs inline `value`; BiDi distinguishes `RemoteValue::Node`, `Array/Object/...` with `handle`, and `Number/String/Boolean/Null/Undefined/BigInt` primitives; WebKit's utility-script wrapper emits a `{kind: 'valueHandle'}` envelope for non-object results and a `{kind: 'handle'}` envelope for retained `window.__wr` entries).
- **Rich-arg walker**: NAPI (`crate::types::NapiEvaluateArg`) and QuickJS (`bindings::convert::quickjs_arg_to_serialized`) detect top-level `JSHandle` / `ElementHandle` class instances at the binding boundary and emit via the shared core helper `JSHandleBacking::to_serialized_argument` — value-backed handles inline as the wire value, remote-backed handles ride as `{h: 0}` plus a `handles[0]` entry. Nested-handle walking (handle inside an object / array user arg) is a non-blocking follow-up.
- **Wire format**: full `protocol::serializers` Playwright-isomorphic round-trip (Date, RegExp, BigInt, NaN, ±Infinity, undefined, -0, typed arrays, ArrayBuffer, Array, Object, Handle, Reference).
- **Known follow-ups** (do not block `[x]`):
  - Nested-handle detection in the rich-arg walker — a `JSHandle` inside an object or array user arg serialises through the JSON fallback today and ends up as an empty object page-side. Top-level handles cover every shipped Playwright test.
  - Observable cleanup test for a CDP `Runtime.releaseObject` failure mid-dispose — the code rolls back the disposed flag on failure, but the retry path is not exercised.
- **Playwright ref**: `packages/playwright-core/src/client/jsHandle.ts`.
- **Files**: `crates/ferridriver/src/js_handle.rs`; NAPI `crates/ferridriver-node/src/js_handle.rs`; QuickJS `crates/ferridriver-script/src/bindings/js_handle.rs`. Wire serializer at `crates/ferridriver/src/protocol/serializers.rs`. Injected utility script at `crates/ferridriver/src/injected/{utilityScript,isomorphic/utilityScriptSerializers}.ts`. Rule-9 tests in `crates/ferridriver-cli/tests/backends_support/handle_surface.rs` and `crates/ferridriver-node/test/handles.test.ts`.
...
#### Partial — signatures landed but per-option integration test still missing

- [~] **Dblclick** — `DblClickOptions` → `ClickOptions { click_count: Some(2) }` lowering. `timeout` propagates via phase-2. Needs: per-option NAPI + backends test that a real `dblclick` handler fires (the lowering is correct but not proven).
- [~] **Press** — `PressOptions { delay, no_wait_after, timeout }`. `delay` wires into keyDown + sleep + keyUp; `timeout` via phase-2. Needs: per-option test that `delay: 120` produces a ≥80ms keyDown/keyUp gap on all 4 backends.
- [~] **Type / pressSequentially** — `TypeOptions { delay, no_wait_after, timeout }`. `delay` wires into per-character press loop; `timeout` via phase-2. Needs: per-option test that `delay: 50` produces a measurable per-char gap.
- [~] **SetInputFiles** — polymorphic `string | string[] | FilePayload | FilePayload[]`. Payloads write to temp files, upload via `set_file_input`, delete after. Needs: per-option test covering each polymorphic form on all 4 backends.
...
4. **Per-option integration tests** — Rule 9: covered for click, addInitScript, tap, fill.force, check/uncheck/setChecked, dispatchEvent timeout, selectOption timeout+force. Still owed for dblclick, press, type, setInputFiles.
...
- **Acceptance for flipping the top-level checkbox**: add the 4 remaining per-option integration tests (dblclick, press, type, setInputFiles) covering every listed option field on every backend. Rule 9 — no signature-only completion claims.
