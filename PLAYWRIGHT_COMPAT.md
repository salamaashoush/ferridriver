...
4. ~~**MCP `run_script` + CDP: `emulateMedia({colorScheme: null})` doesn't update `matchMedia`**~~ — **fixed** alongside 3.24. Root cause: `BrowserState::new` hard-coded `resolve_chromium(false)`, so every MCP-launched browser picked the full Google Chrome binary which retains the macOS system appearance (prefers-color-scheme: dark when the OS is in dark mode), while the direct NAPI `Browser.launch` path threads the actual headless flag through and correctly picks Chrome Headless Shell (always light). CDP wire commands were identical — they just hit two different browsers with different baseline system-appearance behaviour. Fix: new `BrowserState::new_with_headless` constructor that resolves the binary with the correct headless flag up-front; MCP `with_options` switched to the new constructor. `test_script_emulate_media_null_disables_single_field` restored and green on all backends.
