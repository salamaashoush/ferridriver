...
- [x] `toHaveScreenshot(name, options?)` accepts the full Playwright option bag (`mask`, `maskColor`, `animations`, `caret`, `clip`, `scale`, `stylePath`, `maxDiffPixels`, `maxDiffPixelRatio`, `threshold`, `ignore`). Honoured: `threshold` (0–1 → 0–255), `maxDiffPixels`, `maxDiffPixelRatio`, `ignore` (auto-set from `testInfo.ignoreSnapshots`); `animations` (`disabled` injects per-element `animation-duration: 0s` / `transition-duration: 0s`), `caret` (`hide` sets `caret-color: transparent`), `mask` + `maskColor` (overlays the matched selectors with the mask color before capture, `#FF00FF` default), `stylePath` (reads the file and injects its content as a `<style>` tag), `clip` (post-capture crop via the `image` crate). The capture wrapper tags every injected `<style>` with `data-ferridriver-screenshot-capture` and tears them down whether the capture succeeded or failed; `clip` clamps to the captured image bounds. `scale` is accepted but not yet wired through to CDP `Page.captureScreenshot.fromSurface` — Section B follow-up. Wire is in `crates/ferridriver-test/src/expect/locator.rs::capture_with_options`. Rule 9 in `crates/ferridriver-node/test/locator-matcher-options.test.ts` (3 new cases — clip dimensions match the rect, mask paints magenta over the masked region, stylePath injects + cleans up).
...
- [x] `toMatchAriaSnapshot` is backed by Playwright's bundled `InjectedScript.ariaSnapshot()` renderer (already shipped via `injected/dist/engine.min.js`) — the actual aria tree is the canonical YAML-style text Playwright emits. The Rust matcher (`crates/ferridriver-test/src/expect/locator.rs::parse_aria_yaml` + `match_aria_template`) parses both expected and actual into nested `AriaNode` trees and runs Playwright's `matchesNodeDeep` algorithm: each expected root must match some node anywhere in the actual tree (DFS), and the expected node's children must match descendants of the matched actual node — never siblings or descendants of an ancestor. Recognises `[disabled]` / `[checked]` / `[level=2]` bracketed states, literal names, and `/regex/flags` name patterns. Rule 9 in `crates/ferridriver-node/test/locator-matcher-options.test.ts` (3 new cases — ancestor enforcement rejects mis-nested expectations, bracketed disabled state matches, ordered subset still works) plus 6 unit tests in `crates/ferridriver-test/src/expect/locator.rs::aria_tests` (parses simple lines, parses state brackets, enforces ancestor, accepts under correct parent, requires state, matches against subtree).
...
- [x] TS-authored Reporter bridge ships as a `defineReporter(impl)` helper in `packages/ferridriver-test/src/reporter.ts` that collapses a Playwright-shaped `Reporter` object into a single dispatcher function `(payload: { event, args }) => unknown`. The dispatcher is registered via `TestRunner.registerJsReporter(...)` (NAPI), which builds a Rust `JsReporter` (in `crates/ferridriver-node/src/js_reporter.rs`) implementing the core `Reporter` trait and translating each `ReporterEvent` variant into the matching Playwright callback (`onBegin`, `onTestBegin`, `onStepBegin`, `onStepEnd`, `onTestEnd`, `onEnd`, `onWorkerStarted`, `onWorkerFinished`, `onExit`). Playwright-shaped payloads (`TestCase` / `TestResult` / `TestStep` / `FullConfig` / `Suite` / `FullResult`) are emitted via `serde_json::Value` so user code receives plain objects matching the TS types. Multiple TS reporters can be registered — each receives every event. Rule 9 in `crates/ferridriver-node/test/js-reporter.test.ts` (3 cases — full lifecycle dispatch with Playwright-shaped payloads, multi-reporter fan-out, no-op `defineReporter({})`).
